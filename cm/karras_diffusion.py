...
class KarrasDenoiser:
    ...
    def consistency_losses(
        ...
        num_scales,
        model_kwargs=None,
        target_model=None,# versus online model
        teacher_model=None,
...
def karras_sample(
    ...
    def denoiser(x_t, sigma):
        #x_t = x_t.half() if model.dtype==th.float16 else x_t.float()
        _, denoised = diffusion.denoise(model, x_t, sigma, **model_kwargs)
        if clip_denoised:#clip the denoised signal to the range [-1, 1]
...
def sample_for_inverse(
    ...
    x_T = generator.randn(*shape, device=device) * sigma_max
    sample_fn = {"cm": iterative_inverse,
                 "heun": sample_heun_inverse,}[sampler]
    if sampler == "cm":
        sample_args = dict(
...
def to_d_cm(x, sigma, denoised, sigma_min):
    sigma = (sigma**2 - sigma_min**2)**0.5
    return (x - denoised) / append_dims(sigma, x.ndim)
...
def sample_euler(
    ...
    """Implements Algorithm 2 (Heun steps) from Karras et al. (2022)."""
    s_in = x.new_ones([x.shape[0]]) #s_in.shape = [batch_size]
    indices = range(len(sigmas) - 1)
    if progress:
        from tqdm.auto import tqdm
...
def stochastic_iterative_sampler(
    ...
    for i in range(len(ts) - 1):
        t = (t_max_rho + ts[i] / (steps - 1) * (t_min_rho - t_max_rho)) ** rho # t = sigma, same as the sigma
...
def iterative_inverse(  
    ...
    for i in index:
        ...
        x_0 = distiller(x, t * s_in)
        x_0 = th.clamp(x_0, -1.0, 1.0)  
        norm = th.linalg.norm(measurement - operator.forward(x_0, noiseless=True))
        x_y_meta = x_0 - eta * th.autograd.grad(norm, x)[0]
        sigma_s = operator.sigma_s.clip(min=0.001)
        x_var = t.pow(2) / lambda_
        rho = sigma_s.pow(2) / x_var
        tau = rho.float().repeat(1, 1, 1, 1)
        print("tau:",tau)
        print("x_y_meta:",x_y_meta)
        x_y_0 = sr.data_solution(x_y_meta.float(), FB, FBC, F2B, FBFy, tau, sf)
        next_t = sigmas[i+1]
        """
        norm = th.linalg.norm(measurement - operator.forward(x_0))
        x_y_meta = x_0 - eta * th.autograd.grad(norm, x)[0]
        """
        #print(f"next_t{i+1}:", next_t)
        #eps_y = (x - x_y_0).float()
        #zeta = th.tensor(zeta,dtype=th.float32)
        if i < steps-2:
            #print(next_t)
            x = x_y_0 + generator.randn_like(x_y_0.detach()) * th.sqrt(next_t**2 - t_min**2)#+th.sqrt(1-zeta)*eps_y
        else:
            x = x_y_0

    x = distiller(x, t_min * s_in) 
    
    return x

        
@th.no_grad()
def sample_heun_inverse(
    denoiser,
    x,
    sigmas,
    operator,
    ...
    """Implements Algorithm 2 (Heun steps) from Karras et al. (2022)."""
    print("using sample_heun_inverse")
    s_in = x.new_ones([x.shape[0]])
    indices = range(len(sigmas) - 2)
    print(sigmas[-2])
    sf = operator.scale_factor
    FB, FBC, F2B, FBFy = operator.pre_calculated
    sigma_min = sigmas[-2]
    ...
    for i in indices:
        ...
        if gamma > 0:
            x = x + eps * (sigma_hat**2 - sigmas[i] ** 2) ** 0.5
        x_0_1 = denoiser(x, sigma_hat * s_in)
        x_0_1 = th.clamp(x_0_1, -1.0, 1.0)
        sigma_s = operator.sigma_s.clip(min=0.001)
        x_var = sigma_hat.pow(2) / lambda_
        rho = sigma_s.pow(2) / x_var
        tau = rho.float().repeat(1, 1, 1, 1)
        x_0_1_y = sr.data_solution(x_0_1.float(), FB, FBC, F2B, FBFy, tau, sf)
        d = to_d_cm(x, sigma_hat, x_0_1_y, sigma_min)
        """
        if callback is not None:
            callback(
                {
        ...
        """
        dt = sigmas[i + 1] - sigma_hat
        if sigmas[i + 1] == sigma_min:
            # Euler method
            x = x + d * dt 
        else:
            # Heun's method
            x_2 = x + d * dt
            x_0_2 = denoiser(x_2, sigmas[i + 1] * s_in)
            x_0_2 = th.clamp(x_0_2, -1.0, 1.0)
            sigma_s = operator.sigma_s.clip(min=0.001)
            x_var = sigmas[i+1].pow(2) / lambda_
            rho = sigma_s.pow(2) / x_var
            tau = rho.float().repeat(1, 1, 1, 1)
            x_0_2_y = sr.data_solution(x_0_2.float(), FB, FBC, F2B, FBFy, tau, sf)
            d_2 = to_d_cm(x_2, sigmas[i + 1], x_0_2_y, sigma_min)
            d_prime = (d + d_2) / 2
            x = x + d_prime * dt
    return x
...
def sample_heun1_inverse(
    ...
    for i in indices:
        ...
        if sigmas[i + 1] == 0:
            ...
            tau = rho.float().repeat(1, 1, 1, 1)
            x_0_2_y = sr.data_solution(x_0_2.float(), FB, FBC, F2B, FBFy, tau, sf)
            d_2 = to_d(x_2, sigmas[i + 1], x_0_2_y,)
            d_prime = (d + d_2) / 2
            x = x + d_prime * dt
    return x
