...
class InpaintingOperator(LinearOperator):
    ...
    def forward(self, data: torch.Tensor, **kwargs):        
        ...
        y = data.clone()
        if not kwargs.get('noiseless', False):
            y += self.sigma_s * torch.randn_like(y) #add noise before add mask
        return y * self.mask
...
@register_operator(name='hdr')
class HDROperator(NonLinearOperator):
    def __init__(self, device, sigma_s):
        self.device = device
        self.sigma_s = torch.Tensor([sigma_s]).to(device)
    ...
    def forward(self, data, **kwargs):
        ...
        if not kwargs.get('noiseless', False):
            x += self.sigma_s * torch.randn_like(x)
        
        return x

@register_operator(name='phase_retrieval')
class PhaseRetrievalOperator(NonLinearOperator):
    def __init__(self, oversample, device,sigma_s):
    ...
    def forward(self, data, **kwargs):
        ...
        if not kwargs.get('noiseless', False):
            amplitude += self.sigma_s * torch.randn_like(amplitude)   
        return amplitude

@register_operator(name='nonlinear_blur')
class NonlinearBlurOperator(NonLinearOperator):
    def __init__(self, opt_yml_path, sigma_s, device):
        self.device = device
    ...
    def forward(self, data, **kwargs):
        ...
        if not kwargs.get('noiseless', False):
            blurred += self.sigma_s * torch.randn_like(blurred)
        return blurred
